...
switch (process.env.NODE_ENV) {
    case 'development':
        require('dotenv').config({ path: __dirname + '/environment/development/.env' });
        break;

    case 'production':
        require('dotenv').config({ path: __dirname + '/environment/production/.env' });
        break;
...
const { configMongo } = require('./config/microservices');
